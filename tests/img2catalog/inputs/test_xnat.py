...
from img2catalog.inputs.xnat import XNATInput, XNATParserError, filter_keyword
...
def test_xnat_lister(mock_session):
    ...
    with patch.object(
        ...
        assert list_result == [{"name": "p1", "identifier": "project_1"}, {"name": "p3", "identifier": "project_3"}]
...
def test_get_custom_form_metadata_success(project, xnatpy_mock: Mocker, xnatpy_connection: XNATSession):
    ...
    config = {"xnat": {"dataset_form_id": "e2c04eef-6333-41ae-8977-33e2f0793788"}}

    # Mock the custom form API response in the actual format
    custom_form_response = {
        "81057367-f72c-46d6-b6c2-1c409d49f61e": {"condition": "25370001 - Hepatocellular Carcinoma"},
        "9b82df19-6639-4b87-b851-850cbc809c06": {"test_key": False},
        "e2c04eef-6333-41ae-8977-33e2f0793788": {
            "submit": False,
            "creators": [
                {"name": "Prof. C. Reator", "email": "example@example.org", "identifier": "https://example.org"},
                {
...
def test_get_custom_form_metadata_form_not_found(project, xnatpy_mock: Mocker, xnatpy_connection: XNATSession):
    ...
    config = {"xnat": {"dataset_form_id": "nonexistent-form-id"}}
...
def test_get_custom_form_metadata_http_error(project, xnatpy_mock: Mocker, xnatpy_connection: XNATSession):
    ...
    config = {"xnat": {"dataset_form_id": "test-form-id"}}
...
def test_apply_custom_form_metadata_project_not_found(session):
    ...
    metadata_objects = [
        {"uri": "http://localhost/data/archive/projects/nonexistent_project", "title": ["Test Dataset"]}
    ]
...
def test_apply_custom_form_metadata_non_dataset_concept():
    ...
    config = {"xnat": {"catalog_form_id": "test-form-id"}}

    metadata_objects = [{"uri": "http://localhost/catalog", "title": "Test Catalog"}]
...
def test_update_metadata_with_custom_form_add_new_fields():
    ...
    custom_form_data = {"new_field": "New Value", "another_field": ["List Value"]}

    result = xnat_input._update_metadata_with_custom_form(source_obj, custom_form_data)

    expected = {"title": ["Original Title"], "new_field": "New Value", "another_field": ["List Value"]}
...
def test_filter_empty_values_nested_structures():
    ...
    data = {
        "deeply_nested_empty": {"level1": {"level2": {"level3": None}}},
        "deeply_nested_valid": {"level1": {"level2": {"level3": "value"}}},
        "mixed_nested": {"empty_branch": {"all": None}, "valid_branch": {"some": "value"}},
    }
...
def test_parse_custom_form_response_filtering():
    ...
    custom_fields_data = {
        ...
        "other_form_id": {"should_not_appear": "in_result"},
    }
