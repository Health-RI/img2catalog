...
def test_dataset_theme_as_single_string():
    ...
    assert mapped_objects["dataset"][0]["uri"] == URIRef("http://localhost/data/archive/projects/test_img2catalog")
    assert mapped_objects["dataset"][0]["model_object"].theme == [
...
BASE_CATALOG_TEMPLATE = {
    ...
    "contact_point": {"email": "mailto:example@example.com", "formatted_name": "Servicedesk at Example Institution"},
    "dataset": ["http://localhost/data/archive/projects/test_img2catalog"],
}
...
def test_optional_dataset_fields(field_name, field_value):
    ...
    unmapped_objects = {"catalog": [BASE_CATALOG_TEMPLATE], "dataset": [dataset]}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "qualified_relation",
            [{"had_role": ["http://example.com/role1"], "relation": ["http://example.com/relation1"]}],
        ),
...
def test_minimal_required_fields_only():
    ...
    unmapped_objects = {"catalog": [BASE_CATALOG_TEMPLATE], "dataset": [minimal_dataset]}
...
def test_maximum_fields_populated():
    ...
    unmapped_objects = {"catalog": [BASE_CATALOG_TEMPLATE], "dataset": [maximal_dataset]}
    ...
    assert [str(theme) for theme in mapped_dataset.health_theme] == [
        "http://example.com/health1",
...
def test_mixed_optional_fields():
    ...
    unmapped_objects = {"catalog": [BASE_CATALOG_TEMPLATE], "dataset": [mixed_dataset]}
