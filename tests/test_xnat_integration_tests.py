...
TEST_CONFIG_PRIVATE = pathlib.Path(__file__).parent / "xnat_integration_test_config_include_private.toml"
...
@freeze_time("2024-04-01")
@pytest.mark.integration
def test_xnat_integration_include_private(tmp_path, xnat4tests_connection, xnat4tests_uri, isolated_cli_runner,
                                          empty_graph, second_empty_graph):
    """ XNAT Integration test
    ...
    Of these projects only the last three should be serialized.
    """
    # XNAT integration tests
    result = isolated_cli_runner.invoke(cli_click, ["--verbose", "--config", f"{TEST_CONFIG_PRIVATE}",
                                                    "xnat", "--server", xnat4tests_uri, "-u", "admin", "-p", "admin",
                                                    "map-xnat-hriv2",
                                                    "rdf", "-o", f"{tmp_path}/output.ttl"])
    print(result.stdout)
    result_graph = empty_graph.parse(source=f"{tmp_path}/output.ttl")
    reference_graph = second_empty_graph.parse(
        source=pathlib.Path(__file__).parent / "references" / "xnat_integration_test_include_private.ttl")
