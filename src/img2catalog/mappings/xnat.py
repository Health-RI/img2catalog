...
from sempyro.hri_dcat import HRIAgent, HRICatalog, HRIDataset, HRIVCard
...
def get_dict_double_depth(dictionary, first_key, second_key):
    return (
        dictionary[first_key][second_key] if first_key in dictionary and second_key in dictionary[first_key] else None
    )


def get_dict_double_depth_uriref(dictionary, first_key, second_key):
    return (
        URIRef(dictionary[first_key][second_key])
